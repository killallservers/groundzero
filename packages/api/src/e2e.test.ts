...
// Returns one question on first call, then null (DONE)
const mockClarify = mock(async () => null as string | null);
mockClarify.mockImplementationOnce(async () => "Which database should we use?");
...
describe("full pipeline lifecycle", () => {
	test("idea → clarify → answer → run → confirm → generate → download", async () => {
		// 1. Create session
		...
		// 2. Stream extract → clarify (first call returns question)
		...
		const clarifyEvent = streamEvents.find(
			(e) => (e as { type: string }).type === "clarify:question",
		) as { type: string; question: string } | undefined;
		expect(clarifyEvent).toBeTruthy();
		expect(clarifyEvent?.question).toBe("Which database should we use?");

		// 3. Submit answer (second clarify call returns null → advances to resolve)
		const answerRes = await apiRequest(`/sessions/${sessionId}/answer`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
				question: "Which database should we use?",
				answer: "SQLite via Drizzle",
			}),
		});
		expect(answerRes.status).toBe(200);
		const answerBody = await json<{ nextQuestion: string | null }>(answerRes);
		expect(answerBody.nextQuestion).toBeNull();
	...
	test("edit review loops spec back to draft stage", async () => {
		// clarify returns null (DONE) → skip straight to resolve
		const createRes = await apiRequest("/sessions", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
