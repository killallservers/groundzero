	...
	return new Promise((resolve, reject) => {
		...
		es.onmessage = (e) => {
			try {
				...
				onEvent?.(event);
				if (
					event.type === "clarify:question" ||
					event.type === "clarify:done" ||
...
export function App() {
	...
	const handleIdeaSubmit = useCallback(async () => {
		...
		try {
			...
			const clarifyEvent = events.find(
				(e) =>
					e.type === "clarify:question" || e.type === "clarify:done",
			);

			if (!clarifyEvent || clarifyEvent.type === "clarify:done") {
	...
	const handleAnswerSubmit = useCallback(async () => {
		...
		const { sessionId, question, answers } = stage;
		const updated = { ...answers, [question]: answer };
		setAnswer("");

		try {
			setStage({ name: "busy", label: "Thinking…" });
			const { nextQuestion } = await post<{ nextQuestion: string | null }>(
			...
			if (nextQuestion) {
				...
				done(`${count} question${count === 1 ? "" : "s"} answered`);
				await runToReview(sessionId);
			}
		} catch (err) {
			fail(err);
				...
				{stage.name === "idle" && (
					<Card>
						...
						<CardFooter className="justify-between">
							<span className="text-sm text-muted-foreground">⌘↵ to submit</span>
