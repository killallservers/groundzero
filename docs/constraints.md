...
- Do not run `bunx drizzle-kit push` directly — `better-sqlite3` is incompatible with Bun; use `bun db:push` (generate + bun-native migrate)
- Do not use Postgres or any remote database — `drizzle-orm/bun-sqlite` with `bun:sqlite` only
- Do not use a CWD-relative path for the DB file — use `import.meta.url` so it resolves correctly regardless of where the process is launched from
